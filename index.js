...
// Render health check endpoint (prevents sleep)
...
async function connectToWhatsApp() {
    try {
        // Use Render's persistent storage for session
        const sessionPath = process.env.RENDER_VOLUME_PATH || './session';
...
async function startBot() {
    try {
        logger.info('Starting WhatsApp bot on Render...');
        ...
        server.listen(port, '0.0.0.0', () => {
            logger.info(`Bot server started on Render`, {
                port: port,
                botName: config.name,
            ...
            console.log(`🚀 Bot server running on Render port ${port}`);
            console.log(`📱 Open your Render URL to pair WhatsApp`);
            console.log(`🤖 Bot name: ${config.name}`);
            console.log(`🔧 Prefix: ${config.prefix}`);
            console.log(`☁️ Platform: Render`);
        });
        
        // Keep-alive ping for Render free tier (prevents sleep)
        setInterval(async () => {
            try {
                ...
                logger.debug('Keep-alive ping failed (normal during startup)');
            }
        }, 14 * 60 * 1000); // Every 14 minutes (before 15-minute sleep)
        
        // Periodic cleanup (less frequent on Render to save resources)
        setInterval(async () => {
            try {
                const { memoryPlugin } = require('./plugins/memory');
        ...
        }, 3 * 60 * 60 * 1000); // Every 3 hours on Render
